...
test('opus-in-mp4 from flac source: writes a single fragmented mp4 the mp4 scanner can walk', async () => {
  ...
  const target: EncodeTarget = { format: { container: 'mp4', codec: 'opus' }, quality: 'medium' };
...
test('flac-in-mp4 passthrough from flac source: yields a copy-muxed fmp4', async () => {
  ...
  const target: EncodeTarget = { format: { container: 'mp4', codec: 'flac' }, quality: 'max' };
  const handle = spawnEncoder({
    source: SAMPLE_FLAC,
    target,
    outPath: out,
...
test('mp3 from flac source: writes a frame stream the mp3 scanner can walk', async () => {
  ...
  const target: EncodeTarget = { format: { container: 'mp3', codec: 'mp3' }, quality: 'medium' };
  const handle = spawnEncoder({
    source: SAMPLE_FLAC,
    target,
    outPath: out,
...
test('targetKey() produces a stable filesystem-safe key per (codec, quality)', () => {
  expect(
    targetKey({ format: { container: 'mp4', codec: 'opus' }, quality: 'high' }),
  ).toBe('f-opus_q-high');
  expect(
    targetKey({ format: { container: 'mp4', codec: 'flac' }, quality: 'max' }),
  ).toBe('f-flac_q-max');
  expect(
    targetKey({ format: { container: 'mp3', codec: 'mp3' }, quality: 'low' }),
  ).toBe('f-mp3_q-low');
});
