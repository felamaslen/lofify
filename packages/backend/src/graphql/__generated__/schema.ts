...
export function getSchema(config: SchemaConfig): GraphQLSchema {
    ...
    const TrackFormatType: GraphQLInputObjectType = new GraphQLInputObjectType({
        ...
        fields() {
            return {
                autoPassthrough: {
                    description: "Below `MAX`, copy a lossy source the client can play verbatim through at its original quality instead of transcoding it to the requested tier, avoiding a second generation of lossy compression. No effect at `MAX` (already a copy) or on lossless sources (always transcoded to the requested tier).",
                    name: "autoPassthrough",
                    type: GraphQLBoolean
                },
                losslessFormats: {
