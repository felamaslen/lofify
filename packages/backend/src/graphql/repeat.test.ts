...
async function fetchPage(vars: Vars): Promise<Page> {
  const { data } = await gqlRequest(app)
    ...
    .expectNoErrors();
  const conn = data.playbackQueue!.tracks;
  return {
...
test('PlaybackQueue.tracks repeat wraps a step past the last track to the first', async () => {
  await seed(3);
...
test('PlaybackQueue.tracks repeat fills an underfilled page from the start of the order', async () => {
  await seed(3);
  const order = (await fetchPage({ first: 100 })).ids;
...
test('PlaybackQueue.tracks repeat caps a page at one full lap with no duplicates', async () => {
  await seed(5);
  const order = (await fetchPage({ first: 100 })).ids;
...
test('PlaybackQueue.tracks repeat wraps a backward step at the first track to the last', async () => {
  await seed(3);
  const order = (await fetchPage({ first: 100 })).ids;
...
test('PlaybackQueue.tracks repeat cycles the same shuffled permutation', async () => {
  await seed(4);
...
test('PlaybackQueue.tracks repeat reports more pages in both directions whenever tracks match', async () => {
  await seed(2);
  const order = (await fetchPage({ first: 100 })).ids;
...
test('PlaybackQueue.tracks repeat on a single-track set steps back to the same track', async () => {
  await seed(1);
  const order = (await fetchPage({ first: 100 })).ids;
